...
function _getVersionFromGit(experimental: boolean): string {
  ...
  const scmVersionTagRaw = _exec(`git describe --match v[0-9]*.[0-9]*.[0-9]* --abbrev=7 --tags`).slice(1);
...
export const packages: PackageMap = packageJsonPaths
  ...
  .reduce((packages: PackageMap, pkg) => {
    ...
    packages[name] = {
      build: path.join(buildRoot, pkgRoot.substr(packagesRoot.length), 'src'),
      schema: { build: path.join(buildRoot, 'target', pkgRoot.substr(packagesRoot.length), 'src') },
      dist: path.join(distRoot, name),
      root: pkgRoot,
      relative: path.relative(packagesRoot, pkgRoot),
